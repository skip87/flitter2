class User < ActiveRecord::Base
  ...
  # find al friend of that person 
  def friends_of
     Friendship.find(:all, :conditions => ["friend_id= ?", self.id ]).map{|f|f.user}
  end
  # def friends_of
  #     #Friendship.where(:all, :condition => ["friend_id = ?", self.id]).map{|f|f.user}
  #     user_array = Friendship.find(:all, :conditions => ["friend_id= ?", self.id ])
  #     return  User.find(:all , :conditions => ["id in (?)", user_array.map(&:id)])
  #   end
  
  # methode added via the tutorial 
  ...
  def remove_friend(friend)
    ...
    #friendship = friendships.find(:friend_id => friend.id) did not work
    if friendship
      friendship.destroy
    end  
  end
